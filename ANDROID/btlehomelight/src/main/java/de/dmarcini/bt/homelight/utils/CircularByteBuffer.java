 ...
 *   File: CircularByteBuffer.java
 *   Class: de.dmarcini.bt.homelight.utils.CircularByteBuffer
 *
 *   Last modified: 17.09.15 11:08
 *
...
public class CircularByteBuffer
{
  ...
  /**
   * finde das erste Vorkommen eines Zeichens im Puffer
   *
   * @param ch
   * @return
   */
  ...
  {
    int avavible, readPos, count;

    synchronized( CircularByteBuffer.this )
    {
      avavible = available();
      readPos = readPosition; // Schatten...

      //
      // Den Puffer bis zum ersten Vorkommen oder Ende
      // durchkämmen
      //
      //System.out.println( "PUFFER: <" + avavible + "> Bytes, Uebrig: < " + getSpaceLeft() + "> Bytes");
      for( count = 0; count < avavible; count++ )
      {
        if( ch == ( byte ) (buffer[ readPos++ ] & 0x00ff) )
        {
          return (count);
        }
        //
        // Leseposition anpassen, wenn Ende des Puffers wieder zum Anfang
        //
